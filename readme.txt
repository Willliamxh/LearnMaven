...
二部分
 1.maven约定的目录结构， 约定是大家都遵循的一个规则。
  ...
  mvn compile 必须要是在pom文件所在的目录下才能使用
...
第五部分：
  ...
  对于src/main/java 目录下的非java文件(比如a.properties文件)不处理，不拷贝到target/class目录中

  2.2．我们的程序有需要把一些文件放在src/main/java目录中，当我在执行java程序时，需要用到sre/main/java目录中的文件。
  需要告诉maven 在mvn compile src/main/java目录下的程序时，需要把文件一同拷贝到target/classes目录中。
